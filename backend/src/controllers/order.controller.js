...
export const addOrder = AsyncHandler(async (req, res) => {
    ...
    const updateResult1 = await User.updateOne({ _id: user._id }, {
        $push: {
            orders: order._id.toString()
        }
    });
    const updateResult2 = await User.updateOne({ _id: recieverUser._id }, {
        $push: {
            orders: order._id.toString()
        }
    });
    const updateResult3 = await User.updateOne({ _id: "669baf564f903c7d64c4161c" }, {
        $push: {
            orders: order._id.toString()
        }
    });
    const updateResult4 = await User.updateOne({ _id: "669baf994f903c7d64c4161d" }, {
        $push: {
            orders: order._id.toString()
        }
    });
